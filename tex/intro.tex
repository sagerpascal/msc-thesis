...
So far, only the update between two cells is discussed. However, the network consists of multiple cells whose lateral connection strength is defined by a kernel $\boldsymbol{W}$. This kernel has the size $C_{out} \times C_{in} \times n^2+1 \times n^2+1$. Thus, the activity of a cell is calculated based on different weights connecting all neighbouring cells.
...
\begin{align}\eqlbl{hebbu_2}
	\boldsymbol{W(C_{out})} = \boldsymbol{W(C_{out})} / (1\cdot 10^{-10} + \sqrt{\sum_{ci}^{C_{in}}\sum_{w}^{n^2+1}\sum_{h}^{n^2+1} \boldsymbol{W(C_{out},ci,w,h)}})
\end{align}
...
I found that it works best if the weights are initialized with a high self-support.
...
Probleme, dass Probabilities ansteigen (siehe Initialisierung + Verschiedene Anzahl Activations an each location)

Wie Normieren?


\subsubsection{Input over Time}
