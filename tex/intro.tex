...
Such AI systems are trained to optimise a specific target function, such as predicting the subsequent word token in the case of language models \sidecite{radford_improving_2018}.
...
The motivation for this work arises from a comparative analysis of the properties of the biological visual system and deep networks. The following section highlights significant differences and, in particular, identifies shortcomings within neural networks. These identified deficiencies are addressed in \chref{probabilistic_framework} by proposing an alternative approach to current image processing networks.
...
Thus, the first Chapters \chref*{fundamentals}-\chref*{biological_inspiration} survey existing work and put it into context, while the Chapters \chref*{probabilistic_framework}-\chref*{future_work} introduce a novel vision framework.
