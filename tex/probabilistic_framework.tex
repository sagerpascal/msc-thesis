...
The previous chapter introduced neuroscientific findings (\secref{neuroscience_findings}) that could explain why this problem is absent in the human brain and that are considered the principles implementing the findings of the Gestalt psychology \sidecite{ellis_source_1938, kohler_gestalt_1992, wagemans_century_2012, hamlyn_psychology_2017}.
...
Incorporating further stages that implement the long-term vision described in \secref{biologial_inspiration_vision} remains an open task for future research.
...
However, continuous movement is not mandatory, and random data augmentation can also be applied. The network must be informed that the same object with an identical inner structure is shown multiple times during the median loop so that it can learn to map it to the same object prototype despite different transformations and viewpoints.
...
There exist various ways of combining these three types of input signals. For example, the three arrays can be stacked\sidenote{Stacking arrays refers to the process of combining multiple arrays along a specified axis to create a new multidimensional array, e.g. create an array of size $2 \times 2$ out of two arrays with size $1 \times 2$.} to held all arrays available.
...
Please note that the weight between two laterally connected cells is increased if both are active simultaneously, whereby one cell is considered as source cell (denoted as input channel $c'$) of the as target cell (denoted as output channel $c$).
...
During the fast loop when the input is fixed, very fast Hebbian plasticity can be used to create a topographical projection, i.e. to connect neighbouring cells in \emph{S1} with neighbouring cells in \emph{S2} as shown by \sideciteay{fernandes_self-organization_2015}.
