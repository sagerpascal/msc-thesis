\begin{figure}[h]
    \centering
    \includegraphics[width=0.85\textwidth]{sdp_mountain_spirit.jpg}
...
Therefore, some general principles from neuroscience and a unified vocabulary are introduced first.
...
\section{Neuroscientific Inspiration}\seclbl{framework_neuroscience}
Readers of this work are expected to be primarily people with a computer science or neurocomputing background. However, in order to understand the fundamental principles underlying the proposed framework, an understanding of neuroscientific findings and vocabulary is required. Therefore, this section introduces the neuroscientific inspirations and key terms related to this work.
The neuroscience concepts presented in this thesis are simplified to make the proposed framework comprehensible to readers from various fields. Experts in neuroscience may find this section superfluous and may skip it.
...
\paragraph{Lateral Connections and Lateral Support.} The cerebral cortex forms the outer hull of the brain \cite{narr_relationships_2007} and encompasses several regions, including the previously mentioned visual and temporal cortex, as well as the ventral visual stream.
...
\end{figure}
\paragraph{Local Neighbourhood.} Net fragments represent patterns that can be distinguished between local patterns, which are spatially limited to a local area, and global patterns, which extend over larger regions of the image and might encompass the entire image.
...
\paragraph{Overlay of Net Fragments.} A single cell is supported by its neighbouring cells, which, in turn, are supported by their neighbouring cells. Therefore, the support reaches much further than only the local neighbourhood.
...
Thus, the network consists of an overlay of net fragments, which can be interpreted as a larger net fragment, i.e. a multitude of cells supporting each other.
...
This assumption is also backed by the theory of natural intelligence by \citeay{von_der_malsburg_theory_2022}.
...
As described in \secref{framework_building_blocks}, a sensory system can be implemented with hand-crafted filters, Gabor filters \sidecite{gabor_theory_1946, granlund_search_1978}, or by training a convolutional network in an autoencoding setting \sidecite{rumelhart_learning_1986}. The advantage of using learned filters over fixed Gabor filters is that they can be tweaked to the source data. However, this comes with the cost of additional filter training.
...
The human brain's learning algorithm is based on local self-organisation and unsupervised (or self-supervised) learning \sidecite{von_der_malsburg_theory_2022}. The biologically most plausible learning algorithm is Hebbian learning\sidenote{Hebbian learning can be summarised as ``neurons that fire together wire together'' (c.f. \secref{hebbian}).} \sidecite{hebb_organization_1949}.
...
\end{align}
%
In this formula, $\eta$ is the learning rate, and $k_w \in \{0, ..., 2n_l+1\}$ and $k_h\in \{0, ..., 2n_l+1$ is the kernel index along the horizontal and vertical axis\sidenote{For example, $w_{1, 2,3,4}$ represents the weight between output channel $1$ and input channel $2$, located in the kernel's third column and fourth row.}.
...
In the following, \secref{framework_s2_sim} describes how the similarity between neurons connected by a projection fibre can be measured. This statistic is needed to decide whether a control unit should switch on. Subsequently, \secref{framework_s2_mapping} describes how the projection fibres can be wired.
...
\end{figure}
A straightforward approach is to apply different operations such as translation, rotation, and deformation to a reference frame as shown in \figref{prototype_operations}. These operations can be used individually or in combination, resulting in multiple augmented versions of the prototype.
...
For more details on the implementation, please refer to the work by \sideciteay{anderson_shifter_1987} and \sideciteay{olshausen_neurobiological_1993}.
