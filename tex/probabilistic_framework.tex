...
Findings from neuroscience suggest that there is an upper limit of concurrently active incoming synapses for active cells \sidecite{kandel_principles_2013}. Thus, each cell has not only a lower limit of lateral support but also an upper limit.
...
\begin{figure}[h]
    ...
    \figlbl{norm_support}
\end{figure}
The result the function described in \eqref{norm_2} is shown in \figref{norm_support}. As long as a cell's activation is below $\rho$, no normalisation is applied. However, if the activation is above $\rho$, the cell's activation is decreased with a slope of $-0.5$, resulting in a reduction of lateral support.
