...
\paragraph{Conculsion.} There exist various alternative learning approaches with interesting principles. The research identifies the most promising principles from these alternative frameworks and integrates them into a new framework. Sparse coding shows promise in enhancing interpretability and robustness \sidecite{ahmad_properties_2015}. The principle of self-organisation offers the potential to overcome the limitations of current deep learning models that are trained with backpropagation of errors. The iterative process of generating and evaluating hypotheses, as seen in RCNs, is considered essential. Current learning systems often rely on a single forward pass and hastily commit to feature representations without considering the overall structure \sidecite{marr_vision_2010}. When storing representations, the question arises as to the extent of their richness. Actionable representations allow an agent better to understand the behaviour of objects under certain transformations. Storing such representations based on parsimony and self-consistency principles can promote natural intelligence. Finally, combining self-organisation with energy functions can guide the learning process in certain directions so that the agent gains a deeper understanding of its data distribution and acquires reasoning capabilities.
...
Such net fragments could be implemented with the Hebbian rule (c.f. \secref{hebbian}) as it can be used to learn to associate patterns with net fragments. However, there exists a plethora amount of patterns in natural images. Thus, net fragments are not suitable to represent entire objects but shape primitives which can adapt to the shape of actual objects\sidenote{Adapt despite deformations, depth rotation, and position.}.
...
\citeay{von_der_malsburg_theory_2022} have not addressed how their theoretical concepts can be implemented as a mathematical model.
A concrete implementation was proposed by Claude Lehmann in the form of a Master's thesis \sidecite{lehmann_leveraging_2022}.
...
Additionally, such systems have built-in redundancy with a high degree of robustness as they consist of many simpler individual units \sidecite{wagner_19_2013}.
These individual units can even fail without the overall system breaking down.

\paragraph{Grow Patterns.} Cellular automata contain a grid of similar cells\sidenote{An image or features of images can be interpreted as a 2D grid of cells.} with an internal state updated periodically.
...
A recent method called Variational Neural Cellular Automata \sidecite{palm_variational_2022} use an NCA as the decoder of a variational autoencoder \cite{kingma_auto-encoding_2022}.
...
Besides growing 2D patterns, NCAs can also create 3D patterns \sidecite{sudhakaran_growing_2021}, simulate robots \sidecite{hu_regenerating_2021}, or generalize to graph structures \sidecite{grattarola_learning_2021}.
...
\sideciteay{variengien_towards_2021} use the observations of the environment as cell states of an NCA. The following states predicted by the NCA are used as Q-value estimates of a deep Q-learning algorithm \sidecite{Mnih_Kavukcuoglu_Silver_Graves_Antonoglou_Wierstra_Riedmiller_2013}. This allows training RL agents with local self-organisation instead of backpropagation of error.
...
Growing networks overcome these limitations and add nodes or whole layers of nodes into the network structure at the positions of the map where the error is highest \sidecite{fritzke_growing_1994-1, reilly_neural_1982, fritzke_growing_1994, marsland_self-organising_2002}.

Besides ANNs, the structure of spiking neural networks (c.f. Section \secref{spiking_networks}) can also be optimized using self-organisation. 
SNNs can either be assembled from multiple stackable sub-systems \sidecite{raghavan_self-organization_2020}
or be grown from a single cell \sidecite{raghavan_neural_2019} by adding additional cells and connections.


\paragraph{Relevance}
According to \sideciteay{von_der_Malsburg_Stadelmann_Grewe_2022}, self-organisation is the fundamental principle for natural intelligence (c.f. \secref{natural_intelligence}).
...
\sideciteay{risi_future_2021} describes why self-organising systems are hard to train;
...
Soon after backpropagation was published, whether this algorithm is suitable for explaining learning in the brain \sidecite{grossberg_competitive_1987, crick_recent_1989} was questioned.
...
Training end-to-end does not allow controlling the loss function's effect on a hidden layer because of the non-linearity in the network \sidecite{lee_deeply-supervised_2015}.
...
\paragraph{Proxy Objective Functions.} Typical deep learning models are trained by minimizing a global objective function \cite{prince_understanding_2023}. \emph{Proxy objective functions} \sidecite{mostafa_deep_2018, marquez_deep_2018, belilovsky_greedy_2019} are objective functions applied to each layer in the model. For example, the loss function of the first layer (i.e. the proxy objective of the first layer) influences only the trainable parameters of the first layer but not the parameters of the second layer.
...
The same input is fed into the model over multiple timesteps to obtain hierarchical features \sidecite{Hinton_2021}.
