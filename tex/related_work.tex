...
\paragraph{Active Inference.} \citeay{lecun_path_2022} further describes that energy functions are well suited to make predictions of the world state based on actions because they can nicely shape the latent space due to their regulating properties.
...
\sideciteay{von_der_malsburg_theory_2022} point out that the human genome only contains 1GB of information and humans only absorb a few GB into permanent memory over a lifetime \sidecite{mcpherson_physical_2001}. Still, it requires about 1PB to describe the connectivity in the human brain.
...
Therefore, the human brain must be highly structured, and the processes of nature and nurture\sidenote{Nature refers to the influence of genes and evolution, nurture to the influence of experience and education, and emergence to the construction of network structures.} construct the human brain by selecting from a set of pre-structured patterns.
The authors call the process of generating the highly structured network in the human brain the ``Kolmogorov Algorithm \sidecite{kolmogorov_tables_1998} of the Brain''\sidenote{The Kolmogorov complexity describes the number of bits required by the shortest algorithm that can generate a given structure.}.
Network self-organisation is the only mechanism that experiments have not yet disproved as the brains Kolmogorov algorithm \sidecite{willshaw_how_1976, willshaw_marker_1979}.
...
Also, \sideciteay{von_der_malsburg_theory_2022} discuss scene representation (i.e. how a scene is represented in the brain) even though they point out that this is a contested concept \sidecite{freeman_representations_1990}.
...
\citeay{von_der_malsburg_theory_2022} propose that net fragments represent shape primitives which can adapt to the shape of actual objects\sidenote{adapt despite deformations, depth rotation, and position}.
Shifter circuits are one possible implementation of networks that enable invariant responses to the position- and shape-variant representations \sidecite{arathorn_map-seeking_2002, olshausen_multiscale_1995}.
They comprise net fragments that can be formed by network self-organisation \sidecite{fernandes_self-organization_2015}.
...
\citeay{von_der_malsburg_theory_2022} have not addressed how their interesting theoretical concepts can be implemented as a mathematical model.
However, a concrete implementation was proposed by Claude Lehmann in the form of a Master's thesis \sidecite{lehmann_leveraging_2022}.
...
For a long time, this model was state-of-the-art for applications in automatic image analysis, for example, for face recognition \sidecite{XXXXXXXXXXXX}.
...
However, neural networks have weaknesses; for example, they are not transformation-invariant \sidecite{xxxxxxxxx}.
...
% TODO copy stuff from future work that describes how this model works
...
Hebbian learning works well, for example, for topographic mappings\sidenote{mapping input data to a discrete representation} \sidecite{Kohonen_1989, Grajski_Merzenich_1990}, neuroplasticity\sidenote{the network's ability to reorganise itself by forming new connections} \sidecite{Montague_Gally_Edelman_1991}, or recognising non-linear patterns \sidecite{Mel_1992}, but also has limitations \cite{Anderson_1998}.
...
Nokland and Eidnes \sidecite{pmlr-v97-nokland19a} combine the two aforementioned approaches by using two auxiliary networks; one auxiliary network reduces the dimensionality, and another auxiliary network quantifies similarity.
