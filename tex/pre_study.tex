...
The goal of this task is to predict the number (labels $0-9$) that is represented in the image.
...
Figure \figref*{pre_study_components} visualizes these basic components (base lines on the left, digits on the right).
...
An exemplary composition of network fragments of the number $9$ across $4$ subsequent layer is shown in Figure \figref*{pre_study_composition}.
...
The fragments described so far are to be understood as explanatory example.
...
The models are trained on a supervised classification task with the goal to predict the corresponding class of the images of the dataset shown in Figure \figref*{pre_study_dataset}.
...
The models used have different feature extractors consisting of either convolutional (Conv.) layers or fully connected layers (FCN) as shown in Table \tabref*{pre_study_models}.
...
\newcolumntype{R}[1]{>{\raggedleft\let\newline\\\arraybackslash\hspace{0pt}}m{#1}}
    ...
    \centering
	 \begin{tabular}{|l L{9cm}|} 
		...
		9 & FCN ($9*9$ neurons) $\rightarrow$ ReLU $\rightarrow$ head \\ \hline
		10 & FCN ($9*9$ neurons) $\rightarrow$ FCN ($9*9$ neurons) $\rightarrow$ ReLU $\rightarrow$ head \\ \hline
...
Furthermore, it is the only layer that has a global view on the input, i.e. can access all the features extracted by the encoder\sidenote{convolutional layers only consider a local neighbourhood by sliding a kernel over the input (c.f. Section \secref{cnns})} (expect for the encoder consisting of FCN layers only).
...
Blue circles show activations that are exactly $0$, red circles are low activations $>0$, green circles represent strong activations\sidenote{activations $<0$ do not exist because they are set to exactly $0$ by the ReLU function}.
