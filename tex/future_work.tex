...
Nevertheless, training all neurons in a way to optimise consistency at a specific point works exceptionally well for very specific tasks and can outperform human experts \sidecite{buetti-dinh_deep_2019}. Therefore, despite its drawbacks, this algorithm probably remains state-of-the-art for such specialised tasks, while the proposed framework aims to improve generalisation capability and data efficiency, and to approach a human-like understanding of objects.
...
In the context of \emph{S1}, individual cells represent distinct features localised in a specific spatial location. These cells support each other's neuronal activity. As outlined in \secref{neuroscience_findings_alt_cells}, these cells can contribute to mutually exclusive net fragments. For example, cell $A$ may participate in a fragment with cell $B$ and another fragment with cell $C$, while cell $B$ and $C$ avoid simultaneous activation. This exceeds the functional capacity of cell $A$, and a copy of $A$ is needed to establish separate lateral connections with cell $B$ and $C$.
