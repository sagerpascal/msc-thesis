...
Convolutional layers use convolution filters or kernels that slide along input the input and create translation-equivariant\sidenote{The placement of the objects remains consistent between layer input and output, as the same filter is applied to all image positions} \cite{Mouton_Myburgh_Davel_2020} responses known as feature maps \sidecite{Zhang_Itoh_Tanida_Ichioka_1990}.
...
By doing so, CNNs become hierarchical: Since the convolutional operation squeezes information from the surrounding pixel into the output of one pixel, using multiple layers sequentially continuously enlarges the receptive fields\sidenote{the field of input pixels that can influence a single value in the feature map of a layer}.

Pooling layers, on the other hand, reduce the size of the input by conducting dimensionality reduction.
...
These techniques are called weakly-supervised or un-supervised learning and are well summarised ny Simmler et al. \sidecite{Simmler_Sager_Andermatt_Chavarriaga_Schilling_Rosenthal_Stadelmann_2021}.
...
Especially auto-encoders \sidecite{rumelhart1985learning} are explained in this section as they are applied in this thesis.
...
Many recent improvements in deep learning models are due to massively growing model and data set sizes.
...
A recent language model from Microsoft and NVIDIA \sidecite{Smith_Patwary_Norick_LeGresley_Rajbhandari_Casper_Liu_Prabhumoye_Zerveas_Korthikanti_etal} even has \(530\)B parameters.
...
Furthermore, some problems may not be solved with the current principles of deep learning.
...
Backpropagation of error (c.f. Section \secref*{fundamentalsDL}) is the state-of-the-art algorithm that solves this problem by propagating the error signals back through the network.
...
Biological neurons adapt their firing rate to constant inputs, and they may continue firing after an input disappears and can even fire when no input is active (c.f. \secref{neuro_concepts_discrepancy}).
...
An alternative to enforce boundaries is to normalize the length of the weight vector \sidecite{Oja_1982}.
...
Other measures also exist that limit the size of the vector $\boldsymbol{w}$, for example, by using rate-based threshold adaption \sidecite{Bienenstock_Cooper_Munro_1982, Intrator_Cooper_1992}. Rate-based threshold adaption uses a sliding threshold for long-term potentiation (LTP) or long-term depression (LTD) induction.
...
Furthermore, by training a layer of multiple neurons with linear activation, the neurons converge to the first principle component of the input data \cite{Oja_1982}.
...
The negative sum of the term $y_i \cdot a_i$ for the entire network is called the energy function $I(\boldsymbol{y})$\sidenote{typically, the energy function is called $E(\cdot)$ and not $I(\cdot)$; however, $E(\cdot)$ is already used as encoder function in this thesis and therefore $I(\cdot)$ is used as energy function.} of the network:
...
A binary Hopfield network with \(N\) neurons has a capacity\sidenote{the number of patterns that can be stored} of \(C=0.138N\)\sidecite{1057328}.
...
By combining Hebbian learning with some smart mathematical transformations, it can be shown that the weights can be directly learned with only one iteration over the $P$ training patterns\sidenote{for the derivation of this equation refer to \citep{Hopfield_1982}}:
...
Later, the energy function was reformulated as an exponential function \sidecite{Demircigil_Heusel_Löwe_Upgang_Vermet_2017} and thus modern Hopfield networks have an exponential capacity of \(C\approx 2^{\frac{N}{2}}\).
...
The reservoir consists of \(N\) nodes which are connected according to an Erdős–Rényi graph model\sidenote{the Erdős–Rényi model is a model for generating random graphs where all graphs on a fixed set of vertices and edges are equally likely} \sidecite{erdos59a}.
