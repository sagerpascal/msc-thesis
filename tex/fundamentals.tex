...
Accordingly, \secref{neurons} provides a brief explanation of how biological neurons and learning work and relates them to their artificial counterparts.
...
\sideciteay{mcculloch_logical_1943} proposed the first model of a neuron that can be connected to other neurons.
...
\end{align}
%
In 1958, \sideciteay{rosenblatt_perceptron_1958} proposed the perceptron, which extends the neuron with learnable weights: The input vector $\boldsymbol{x}$ of length $n$ is multiplied with a weight vector $\boldsymbol{w} \in \mathbb{R}^n$ of the same length.
%
