...
\subsubsection{Overlay of Net Fragments}
...
\section{Vision}\seclbl{biologial_inspiration_vision}
In the following, a vision is described how these findings could serve as the foundation to build a novel vision framework\sidenote{Much of the vision is inspired by self-organising projection fibres (c.f. \secref{projection_fibres}) and inputs by leading experts from the field that supervise this thesis.
However, the vision is adapted to some extent according to my personal view and not entirely backed by existing scientific findings - that's why it is a vision.}.
The previous section introduced promising neuroscientific findings to build a novel vision framework.
The core behind this vision is based on two stages and projection fibres connecting them.
The first stage builds net fragments reassembling a visual scene captured with our eyes. The lateral reach within this stage is limited since many pattern combinations can be experienced in the real world. However, scenes are dominated by locally similar-looking patterns that can be captured with net fragments.
...
If, for example, for each object rotated by 30°, a different object exists, information about the object's rotation is still available in the second stage but with a lower resolution than in the first stage.

Projection fibres connect these two stages, mapping objects within a visual scene to object prototypes.
This mapping is not intended as a classification layer but rather as a scene interpretation layer: A classification layer would map a pattern, for example, to a specific person with a name. In contrast, projection fibres map the pattern to a more generalised view of this person, i.e. transforming it into a reference person. A subsequent memory stage would be needed to map such a reference person to an actual label, such as the person's name. However, this memory has yet to be investigated in detail within this thesis and remains an open question for future research.

\subsection{Scene Interpretation}
As described in \secref{neuroscience_findings_local_learning_principle}, the human brain optimises consistency at every point in the network.
This is considered a key principle that has to be integrated into a computational framework: Consistency is optimised between the cells within the two stages to build net fragments but also between the projection fibres connection net fragments to obtain a valid mapping.
...
For instance, projection fibres could map one object in the scene to a person while mapping another object in close spatial proximity of this person's foot to a ball. By building consistency between these two objects, one could conclude that this person is most likely playing football (soccer). If such scenes are observed several times, synaptic connections between these references could be formed, allowing for scene interpretation.
...
One of the strengths of deep networks is that they build features hierarchically \sidecite{prince_understanding_2023}. Hierarchical processing allows a step-wise mapping of specific data, such as visual scenes, to more abstract representations, such as labels or segmentation masks.
...
As described in \secref{neuroscience_findings}, cortical columns also consist of multiple layers.
However, these layers serve different functionalities and are required, for example, to deal with depth rotation rather than extracting higher-level features hierarchically. 
