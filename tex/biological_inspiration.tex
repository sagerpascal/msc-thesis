...
However, the visual cortex only detects patterns in visual data but does not draw conclusions from it. Instead, it forwards it as an information stream to other brain areas. In this thesis, especially the ventral visual stream \sidecite{goodale_separate_1992} is of importance. This stream forwards the detected patterns to the temporal cortex \sidecite{miyashita_inferior_1993, conway_organization_2018}, a brain region located behind the ears.
...
Since cells are often active together when representing the same pattern, lateral support is increased between groups of neurons representing frequently occurring patterns \sidecite{stettler_lateral_2002}. Such groups are called \emph{net fragments}.
...
To capture frequently occurring patterns, the visual cortex limits the range of lateral connections to a \emph{local neighbourhood} \sidecite{stettler_lateral_2002, liang_interactions_2017}.
...
\subsection{Correspondence Finding}\seclbl{neuroscience_findings_correspondence_finding}
...
Building consistency means that the cells reach a consensus on what they represent: Features trigger cells belonging to specific net fragments and only remain active if other cells contribute to the same fragment and provide mutual support. Cells that do not receive sufficient support are deactivated by inhibition \sidecite{vogels_inhibitory_2011}
...
\section{Computational Framework: A Vision}\seclbl{biologial_inspiration_vision}
...
In \chref{probabilistic_framework}, a framework is proposed that implements these two stages with projection fibres.
...
As described in \secref{neuroscience_findings_local_learning_principle}, consistency is optimised between the cells within the two stages to build net fragments and between the projection fibres connecting net fragments.
