...
The aforementioned neuroscientific findings serve as foundation to build a novel image-processing framework\sidenote{Much of this vision is inspired by self-organising projection fibres (c.f. \secref{projection_fibres}) and inputs by leading experts from the field that supervise this thesis.}.
...
It is considered as a computational implementation of the foundation of the biological visual system.
...
Some evidence suggests that the layers in cortical columns could deal with depth rotations \sidecite{iamshchinina_perceived_2021}, implementing hierarchical processing in different stages \sidecite{felleman_distributed_1991}.
...
Second, projection fibres are responsible for mapping specific scenes to general reference frames, implementing the process that deep networks achieve through layer-wise forward propagation. These fibres are hierarchically organised as outlined in work about shifter circuits \sidecite{anderson_shifter_1987, olshausen_neurobiological_1993}. 
