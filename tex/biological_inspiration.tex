...
In \figref{maplets}, such a projection between net fragments and an object prototype of a line is visualised. A vast amount of projection fibres (grey) run between these two areas, but only the most suitable ones are activated by the control unit of a maplet (red).
...
In \figref{maplets}, a direct mapping from net fragments to prototypes is shown for better clarity. In contrast, the mapping in the brain is done over several hierarchical levels, saving many projection fibres \sidecite{anderson_shifter_1987}.
