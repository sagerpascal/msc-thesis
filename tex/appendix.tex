...
It was observed that Hebbian learning sometimes works well for tiny networks but does not scale well for larger networks or more complex datasets.
...
Thus, net fragments improve interpretability and lead to sparse activations\sidenote{since each small only contains a (small) sub-set of the learned features, only a (small) sub-set of neurons should be active}, which could increase robustness.
...
A very intuitive way for humans to construct features on this dataset is to interpret each line as a feature.
For example, the $10$ different digits in this dataset could be composed of $9$ basic lines.
Thus, a low-level net fragment could represent a basic line and be combined to higher-level net fragments like digits.
\figref{pre_study_components} visualises such an intuitive composition (basic lines on the left, digits on the right).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.99\textwidth]{pre_study_components}
    \caption[Line types in the straight line digits dataset]{A visualisation of all the lines (left) needed to compose the digits in the dataset (right). The coloured lines in-between illustrate the relationship between the lines and the digits.}
...
The visualisation in \figref{pre_study_components} can be interpreted as a composition across one layer, i.e. the low-level fragments are combined to high-level fragments within one step.
An exemplary composition of net fragments of the number $9$ across $4$ subsequent layers is shown in \figref{pre_study_composition}.
...
\begin{figure}[h]
    ...
    \caption[Composition of net fragment]{A sample composition of the net fragment that could represent the digit $9$.}
...
Two types of models are trained on this dataset, namely a classification network (supervised) and an autoencoder network (unsupervised).
The goal of the classification task is to predict the number (labels $0-9$) that is shown in the image (c.f. \secref{train_cnns}).
...
Contrarily, an autoencoder works unsupervised and reconstructs the input data from a lower dimensional embedding space (c.f. section \secref*{visual_rep_learning}).
...
The model's parameters are optimised by minimising the cross-entropy loss with the Adam optimiser \sidecite{Kingma2015AdamAM} so that the models learn to predict the corresponding class of the images shown in \figref{pre_study_dataset}.
...
It can be seen in the visualisation that none of the models utilises all $12$ neurons, and certain neurons are always inactive regardless of the class.
...
Autoencoders differ not only in the design of the network (i.e. number and type of layers) but also in the constraints applied in the bottleneck layer (c.f. \secref{visual_rep_learning_ae}).
...
The two autoencoder architectures used are shown in \tabref{pre_study_ae_models}.
...
This can be observed in \figref{pre_study_ae1} that shows the activations of the ``model 1''.
...
Figure \figref*{pre_study_ae2} (b) shows which feature the $18$-th neuron represents. Together with the $33$-th neuron, this neuron represents the number 5 and the $36$-th neuron, the number 9.
